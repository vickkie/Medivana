...
const Checkout = () => {
  ...
  const handleSubmitOrder = async (paymentInfo) => {
    ...
    try {
      ...
      if (
        ...
        const initRes = await axios.post(`${BACKEND_PORT}/api/v6/payments/initialize`, {
          amount: estimatedAmount,
          email: paymentInfo?.email || email,
          appointmentId,
          userId: userData?._id,
          callback_url: `${BACKEND_PORT}/payment/callback`,
        });

        if (!initRes.data?.success) {
          showToast("error", initRes.data?.message || "Failed to initialize payment");
          setIsLoading(false);
          return;
        }
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {showPaystackWebView && paystackUrl && (
        <Modal
        ...
        >
          <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
            ...
            >
              ...
              <Text style={{ fontSize: 18, fontWeight: "600", color: "#333" }}>Complete Payment</Text>
            </View>
            <WebView
              ...
              onNavigationStateChange={async (navState) => {
                ...
                if (url.includes("/payment/callback") || url.includes("reference=")) {
                  // Extract reference from URL
                  const refMatch = url.match(/reference=([^&]+)/);
                  const ref = refMatch ? refMatch[1] : paystackReference;

                  if (ref) {
                    try {
                      // Verify payment
                      const verifyRes = await axios.get(`${BACKEND_PORT}/api/v6/payments/verify/${ref}`);

                      if (verifyRes.data?.status === "success") {
                        setShowPaystackWebView(false);
                        showToast("success", "Payment successful!");
                        navigation.navigate("OrderSuccess", { orderId: ref });
                      } else {
              ...
              onError={(error) => {
                ...
                setShowPaystackWebView(false);
                setIsLoading(false);
                showToast("error", "Payment page failed to load");
              }}
            />
          </SafeAreaView>
        </Modal>
